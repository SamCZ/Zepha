...
Api::Usertype::AnimationManager Api::Usertype::AnimationManager::define(sol::table anims) {
	...
	for (auto& ref : anims) {
		...
		animations.push_back(
			{ n, { static_cast<unsigned int>(t.get<float>(1)), static_cast<unsigned int>(t.get<float>(2)) }});
	}
...
Api::Usertype::AnimationManager Api::Usertype::AnimationManager::set_anim(sol::object anim, sol::optional<bool> loop) {
	if (anim.is<std::string>()) entity->animation.setAnim(anim.as<std::string>(), 0, loop.value_or(true));
		...
		entity->animation.setAnim(
			glm::ivec2{ anim.as<sol::table>().get<unsigned int>(1), anim.as<sol::table>().get<unsigned int>(2) }, 0,
			loop.value_or(true));
	return *this;
}
...
Api::Usertype::AnimationManager
Api::Usertype::ServerAnimationManager::set_anim(sol::object anim, sol::optional<bool> loop) {
