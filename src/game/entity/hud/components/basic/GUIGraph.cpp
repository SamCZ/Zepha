...
void GUIGraph::buildHistogramMesh() {
    ...
    for (float num : history) {
        ...
        auto columnVerts = std::vector<GuiVertex> {
            {{xOffset,    -h}, {uv.x + age * uv.z,         uv.y + sec * uv.w,         0, 0}, {1, 1, 1}, true},
            {{xOffset + 1,-h}, {uv.x + (age+0.01f) * uv.z, uv.y + sec * uv.w,         0, 0}, {1, 1, 1}, true},
            {{xOffset + 1, 0}, {uv.x + (age+0.01f) * uv.z, uv.y + (sec+0.10f) * uv.w, 0, 0}, {1, 1, 1}, true},
            {{xOffset,     0}, {uv.x + age * uv.z,         uv.y + (sec+0.10f) * uv.w, 0, 0}, {1, 1, 1}, true},
        };
